...
exports.CalendarLunar = void 0;
const react_1 = require("react");
const react_2 = __importDefault(require("react"));
require("./calendar.css");
const constant_1 = require("../../../constant");
const CalendarLunar = ({ onClick }) => {
    ...
    function getYearInfo(yyyy) {
        var yearCode;
        if (yyyy < 1300) {
            yearCode = constant_1.TK13[yyyy - 1200];
        }
        else if (yyyy < 1400) {
            yearCode = constant_1.TK14[yyyy - 1300];
        }
        else if (yyyy < 1500) {
            yearCode = constant_1.TK15[yyyy - 1400];
        }
        else if (yyyy < 1600) {
            yearCode = constant_1.TK16[yyyy - 1500];
        }
        else if (yyyy < 1700) {
            yearCode = constant_1.TK17[yyyy - 1600];
        }
        else if (yyyy < 1800) {
            yearCode = constant_1.TK18[yyyy - 1700];
        }
        else if (yyyy < 1900) {
            yearCode = constant_1.TK19[yyyy - 1800];
        }
        else if (yyyy < 2000) {
            yearCode = constant_1.TK20[yyyy - 1900];
        }
        else if (yyyy < 2100) {
            yearCode = constant_1.TK21[yyyy - 2000];
        }
        else {
            yearCode = constant_1.TK22[yyyy - 2100];
        }
    ...
    function printHead(mm, yy) {
        var res = '';
        var monthName = constant_1.MONTHS[mm - 1] + ' ' + yy;
        res += '<tr class="t-head" >\n';
        res +=
        ...
        res += '<tr>\n';
        for (var i = 0; i <= 6; i++) {
            res += '  <td class="ngaytuan">' + constant_1.DAYNAMES[i] + '</td>\n';
        }
        res += '</tr>\n';
        return res;
    }
    function checkHolidaySolar(dd, mm) {
        var res = '';
        constant_1.LE.solar.forEach((item) => {
    ...
    function checkHolidayLunar(dd, mm) {
        var res = '';
        constant_1.LE.lunar.forEach((item) => {
            if (item.d == dd && item.m == mm) {
    ...
    return (react_2.default.createElement("div", { className: 'wrapper-calendar-lunar' },
        react_2.default.createElement("div", { className: 'calendar-lunar' })));
};
exports.CalendarLunar = CalendarLunar;
